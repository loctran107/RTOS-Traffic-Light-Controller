...
//  xTaskCreate(Control, (const char *)"Control FSM", 1024, NULL, , NULL);
  ...
  xTaskCreate(StartStop, (const char *)"StartStopButton", 1024, NULL, 15, NULL); //middle priority
  xTaskCreate(Pedestrian, (const char *)"PedestrianButton", 1024, NULL, 15, NULL); //maybe highest priority
  xTaskCreate(Control, (const char *)"Control FSM", 1024, NULL, 14, NULL); //lowest priority
...
void StartStop(void *p) {
  ...
  while (1) {
    curr_onoff_tick_time = xTaskGetTickCount();
    printf(curr_onoff_tick_time- prev_onoff_tick_time);
    printf(prev_onoff_tick_time);
...
void Control(void *p) {
  ...
  while (1) {
    ...
    if (s == stop && next_state == go) {
      ...
      if ((onoff_status == 1 && pedestrian_status == 0) || x == execute){
        red_led_on(); //red led on
        if (curr_led_tick_time - prev_led_tick_time >= 5000) {
    ...
    if ((s == go) && (next_state == warn)) { 
      if (onoff_status == 1 && pedestrian_status == 0) {
    ...
    if (s == warn && next_state == stop) {
      ...
      if (onoff_status == 0 && pedestrian_status == 1) {
        yellow_led_on();
      }
      if (curr_led_tick_time - prev_led_tick_time >= 5000) {
